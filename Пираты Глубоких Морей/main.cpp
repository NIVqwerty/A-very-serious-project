...
void bar(int x, int y)
  ...
  txRectangle(x+-200-0,450,x+200-0,600);
    ...
    txDrawText(x+460-0,y+265-450,x+729-0,y+325-450, "Бар");
     ...
     // const char* text чтобы менять текст в мэйне - это экономит место
     // sizeFont меняет шрифт в мэйне - это экономит место

void Titrs(const char* text, int sizeFont)
     {
...
void Replic(int x, int y, const char* text, int sizeFont)
     ...
     txSelectFont("Comic Sans MS" , sizeFont);
     txDrawText (x,y,x+500,y+100,text);
     }
...
int main()
    ...
    table();
    Man(xMan,yMan,xLegs);
    Replic(300, 70, "Привет, слышал тебе нужен помощник", 25);
    txEnd();
    xPoint = xPoint + 1;
    txSleep(20);
    ...
    table();
    Man(xMan,yMan,xLegs);
    Replic(300, 70, "Я могу тебе в этом помочь, но за небольшой процент", 25);
    txEnd();
    xPoint = xPoint + 1;
    txSleep(20);
